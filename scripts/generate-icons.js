// PWA ikonlarını oluştur
const fs = require('fs');
const path = require('path');

// SVG logo'yu kullanarak basit bir ikon oluştur
const iconSvg = `
<svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="grad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" style="stop-color:#9333ea;stop-opacity:1" />
      <stop offset="100%" style="stop-color:#ec4899;stop-opacity:1" />
    </linearGradient>
  </defs>
  <rect width="512" height="512" rx="100" fill="url(#grad)"/>
  <g transform="translate(256, 256)">
    <!-- Kalp şekli -->
    <path d="M0,-80 C-40,-120 -120,-120 -120,-40 C-120,20 0,100 0,100 C0,100 120,20 120,-40 C120,-120 40,-120 0,-80 Z" 
          fill="white" opacity="0.9"/>
    <!-- Artı işareti -->
    <rect x="-8" y="-60" width="16" height="120" fill="white" rx="8"/>
    <rect x="-60" y="-8" width="120" height="16" fill="white" rx="8"/>
  </g>
</svg>
`;

// Public klasörüne kaydet
const publicDir = path.join(__dirname, '..', 'public');

// 192x192 ikon
fs.writeFileSync(
  path.join(publicDir, 'icon-192.png'),
  Buffer.from(iconSvg.replace('width="512" height="512"', 'width="192" height="192"'))
);

// 512x512 ikon
fs.writeFileSync(
  path.join(publicDir, 'icon-512.png'),
  Buffer.from(iconSvg)
);

console.log('✅ PWA ikonları oluşturuldu!');
