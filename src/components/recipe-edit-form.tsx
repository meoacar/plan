...
export default function RecipeEditForm({ recipe }: any) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [uploadingImage, setUploadingImage] = useState(false);
  const [images, setImages] = useState<string[]>(
    recipe.images.map((img: any) => img.url)
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Tarifiniz güncellendi ve tekrar onaya gönderildi!");
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <div className="grid gap-4 sm:grid-cols-4">
          ...
          <div>
            <label className="mb-2 block text-sm text-gray-700">Yağ (g)</label>
            <input
              type="number"
              min="0"
              step="0.1"
      ...
      <div className="flex gap-4">
        ...
        >
          {loading ? "Güncelleniyor..." : "Güncelle ve Onaya Gönder"}
        </button>
        <button
          type="button"
          onClick={() => router.back()}
